...
Paper runs (five-condition supervision, trace-reliability sweeps, LoRA adaptation, figures): **[RUN.md](RUN.md)**.
...
| Command | Purpose |
|---|---|
| `python -m src lora` | LoRA adaptation of a pretrained causal LM (SmolLM2-135M by default) on the same supervision formats |
| `python experiments/run.py <command>` | Figure/table builders and closed-form checks that need no training (noise threshold, credit suppression, clean convergence, Proposition 8 frontier, fraction-vs-amount, figure export) — see [experiments/README.md](experiments/README.md) |
...
The paper's tasks are Boolean circuits (`boolean_circuit_*`). The registry also carries other sequential-executor task families (finite-state/register machines, modular programs, stack machines, word-index problems) that are exercised by the test suite but are not part of the current paper's claims.
...
| Location | Contents |
|---|---|
| [experiments/](experiments) | One CLI, `python experiments/run.py <command>`. **[experiments/README.md](experiments/README.md)** maps commands to claims. |
| [handcoded/](handcoded) | The constructive Transformer executors and the tutorial notebook |
...
| [docs/assets/](docs/assets) | Tracked preview media for the tutorial animation |
...
|---|---|
| `results/paper/` | `python -m src supervision` (Table 1 rerun), `python -m src lora` (LoRA study), `python -m src reliability` (depth × reliability) |
| `results/reliability_sweeps/` | `python -m src reliability` (source data for Figure 1/2) |
| `results/noise_threshold/`, `results/credit_suppression/`, `results/clean_convergence/`, `results/fraction_vs_amount/`, `results/prop8_frontier/` | `python experiments/run.py <command>` (no training; closed-form/numerical checks) |
