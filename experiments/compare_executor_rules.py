...
sys.path.insert(0, str(ROOT / 'handcoded'))
import handcoded_utils as h
...
def write_report(outdir, args, records):
    ...
    lines += ['', '## Scope', '',
              ...
              '## Reproduce', '', '```bash',
              'python experiments/compare_executor_rules.py --output results/executor_comparison/reproduction '
...
def main():
    ...
    parser.add_argument('--output', type=Path, default=ROOT / 'results/executor_comparison/pilot')
    for name, default in [('depth', 4), ('seed', 42), ('steps', 2000), ('train-size', 10000),
                          ('test-size', 256), ('probe-size', 64), ('batch-size', 128),
                          ('d-model', 96), ('d-ff', 192), ('threads', 2), ('backgrounds', 2)]:
        parser.add_argument('--' + name, type=int, default=default)
    parser.add_argument('--lr', type=float, default=0.002)
    parser.add_argument('--device', default='cpu')
    parser.add_argument('--checkpoints', type=int, nargs='+', default=[0, 100, 500, 1000, 2000])
    ...
    optimizers = {mode: torch.optim.AdamW(model.parameters(), lr=args.lr, weight_decay=0.01)
                  for mode, model in models.items()}
    ...
    manifest = {**vars(args), 'output': str(args.output), 'torch_version': str(torch.__version__),
    ...
    for step in range(args.steps + 1):
        ...
        for mode, model in models.items():
            ...
            optimizer.step()
        if (step + 1) % 100 == 0:
            print(f'trained {step + 1}/{args.steps}; elapsed {time.monotonic() - started:.1f}s', flush=True)
