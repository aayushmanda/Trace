...
# FROZEN SNAPSHOT (results/mechanism_deep/frozen_code/mechanism_pipeline.py):
# taken from src/eval/local_credit.py while that file was under active
# concurrent edits from another session on this same repo/machine, to
# decouple this study's launches from further mid-flight changes (an earlier
# change there introduced a bug -- heterogeneous metrics.csv row keys crashed
# csv.DictWriter -- that silently lost 19/20 full training runs at the final
# write_csv call). See results/mechanism_deep/SUMMARY.md for the full story.
# This file intentionally duplicates src/eval/local_credit.py; do not "clean
# up" the duplication without checking that note first.
...
import nnsight  # mentioned in persist.json / report.md only; not on the patch path below

ROOT = Path("/home/hariguru/aayus/trace")
if str(ROOT) not in sys.path:
    sys.path.insert(0, str(ROOT))
_FROZEN_DIR = Path(__file__).resolve().parent
if str(_FROZEN_DIR) not in sys.path:
    sys.path.insert(0, str(_FROZEN_DIR))
...
from handcoded_local_credit import (
    chance_accuracy,
    collect_hiddens,
