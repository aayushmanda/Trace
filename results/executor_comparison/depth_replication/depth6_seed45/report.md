...
Fresh pilot: depth 6, seed 45, 2,000 updates, 10,000 unique training circuits and 256 disjoint held-out circuits.
...
| outcome | 7.0% | not queried | not defined |
| both | 18.0% | 100.0% | 0.902 |
...
Final mixed-format full-gradient cosine: **0.058**; relative gradient error: **1.034**. These measurements do not by themselves establish a shared mechanism.
...
```bash
python experiments/compare_executor_rules.py --output results/executor_comparison/reproduction --depth 6 --seed 45 --steps 2000 --train-size 10000 --test-size 256 --probe-size 64 --batch-size 128 --d-model 96 --d-ff 192 --lr 0.002 --device cpu --threads 1 --backgrounds 2 --checkpoints 0 100 500 1000 2000
```
