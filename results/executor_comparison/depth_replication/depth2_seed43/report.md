...
Fresh pilot: depth 2, seed 43, 2,000 updates, 10,000 unique training circuits and 256 disjoint held-out circuits.
...
| outcome | 74.2% | not queried | not defined |
| both | 76.2% | 99.9% | 0.452 |
...
Final mixed-format full-gradient cosine: **0.053**; relative gradient error: **0.999**. These measurements do not by themselves establish a shared mechanism.
...
```bash
python experiments/compare_executor_rules.py --output results/executor_comparison/reproduction --depth 2 --seed 43 --steps 2000 --train-size 10000 --test-size 256 --probe-size 64 --batch-size 128 --d-model 96 --d-ff 192 --lr 0.002 --device cpu --threads 1 --backgrounds 2 --checkpoints 0 100 500 1000 2000
```
