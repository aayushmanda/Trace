...
Fresh pilot: depth 2, seed 42, 2,000 updates, 10,000 unique training circuits and 256 disjoint held-out circuits.
...
| outcome | 61.3% | not queried | not defined |
| both | 48.4% | 100.0% | 0.819 |
...
Final mixed-format full-gradient cosine: **0.013**; relative gradient error: **1.004**. These measurements do not by themselves establish a shared mechanism.
...
```bash
python experiments/compare_executor_rules.py --output results/executor_comparison/reproduction --depth 2 --seed 42 --steps 2000 --train-size 10000 --test-size 256 --probe-size 64 --batch-size 128 --d-model 96 --d-ff 192 --lr 0.002 --device cpu --threads 1 --backgrounds 2 --checkpoints 0 100 500 1000 2000
```
