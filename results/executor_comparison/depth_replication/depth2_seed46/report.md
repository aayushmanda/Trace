...
Fresh pilot: depth 2, seed 46, 2,000 updates, 10,000 unique training circuits and 256 disjoint held-out circuits.
...
| outcome | 65.6% | not queried | not defined |
| both | 78.9% | 100.0% | 0.503 |
...
Final mixed-format full-gradient cosine: **0.170**; relative gradient error: **0.986**. These measurements do not by themselves establish a shared mechanism.
...
```bash
python experiments/compare_executor_rules.py --output results/executor_comparison/reproduction --depth 2 --seed 46 --steps 2000 --train-size 10000 --test-size 256 --probe-size 64 --batch-size 128 --d-model 96 --d-ff 192 --lr 0.002 --device cpu --threads 1 --backgrounds 2 --checkpoints 0 100 500 1000 2000
```
