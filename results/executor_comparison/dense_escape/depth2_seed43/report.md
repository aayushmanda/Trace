...
Fresh pilot: depth 2, seed 43, 200 updates, 10,000 unique training circuits and 256 disjoint held-out circuits.
...
|---|---:|---:|---:|
| process | 97.7% | 99.4% | 0.074 |
| outcome | 43.4% | not queried | not defined |
| both | 34.8% | 69.7% | 0.318 |
...
Final mixed-format full-gradient cosine: **0.525**; relative gradient error: **1.211**. These measurements do not by themselves establish a shared mechanism.
...
```bash
python -m src executor --output results/executor_comparison/reproduction --depth 2 --seed 43 --steps 200 --train-size 10000 --test-size 256 --probe-size 64 --batch-size 128 --d-model 96 --d-ff 192 --lr 0.002 --device cuda:0 --threads 1 --backgrounds 2 --checkpoints 0 5 10 15 20 25 30 40 50 65 80 100 130 160 200
```
