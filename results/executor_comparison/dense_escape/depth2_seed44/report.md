...
Fresh pilot: depth 2, seed 44, 200 updates, 10,000 unique training circuits and 256 disjoint held-out circuits.
...
|---|---:|---:|---:|
| process | 96.9% | 98.7% | 0.089 |
| outcome | 35.2% | not queried | not defined |
| both | 39.5% | 69.4% | 0.250 |
...
Final mixed-format full-gradient cosine: **0.468**; relative gradient error: **1.128**. These measurements do not by themselves establish a shared mechanism.
...
```bash
python -m src executor --output results/executor_comparison/reproduction --depth 2 --seed 44 --steps 200 --train-size 10000 --test-size 256 --probe-size 64 --batch-size 128 --d-model 96 --d-ff 192 --lr 0.002 --device cuda:0 --threads 1 --backgrounds 2 --checkpoints 0 5 10 15 20 25 30 40 50 65 80 100 130 160 200
```
