...
Fresh pilot: depth 4, seed 43, 200 updates, 10,000 unique training circuits and 256 disjoint held-out circuits.
...
|---|---:|---:|---:|
| process | 94.1% | 98.9% | 0.144 |
| outcome | 13.3% | not queried | not defined |
| both | 9.4% | 76.3% | 0.405 |
...
Final mixed-format full-gradient cosine: **0.312**; relative gradient error: **2.012**. These measurements do not by themselves establish a shared mechanism.
...
```bash
python -m src executor --output results/executor_comparison/reproduction --depth 4 --seed 43 --steps 200 --train-size 10000 --test-size 256 --probe-size 64 --batch-size 128 --d-model 96 --d-ff 192 --lr 0.002 --device cuda:0 --threads 1 --backgrounds 2 --checkpoints 0 5 10 15 20 25 30 40 50 65 80 100 130 160 200
```
