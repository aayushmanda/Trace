"""Pretrained LoRA adaptation on the Boolean-circuit task (paper §3, SmolLM2-135M).

Reproduces the pretrained-model reliability sweep and 16-way local-state
scoring diagnostic reported in the paper (LoRA rank 8 on all linear layers,
depth-8 Boolean circuits, rho in {0, 0.2, 0.4, 0.5, 0.6, 0.8, 1.0}, seeds
2001-2003). Trains OUTCOME, optionally ANSWER-FIRST, and one PROCESS
condition per rho, evaluates free-running generation, and for PROCESS
conditions additionally scores the 16-way local-state margin at every gold
prefix. This is the pretrained path: not the from-scratch GPT stack
(`python -m src supervision` / `python -m src reliability`) and not the
handcoded semantic-token executors (`python -m src executor`).

Usage:
  python -m src lora --config configs/experiments/lora_transfer.yaml
  python -m src lora --model HuggingFaceTB/SmolLM2-135M --task boolean_circuit_8 \
      --rhos 0.5 --seeds 2001 --train-size 200 --steps 20 --overwrite
"""
...
class CompletionDataset(Dataset):
    def __init__(self, instances, tokenizer, condition, rho, assignment_order, max_length):
        ...
        if condition == "process":
            if rho is None:
                raise ValueError("process requires rho")
        ...
        for i, inst in enumerate(instances):
            completion = (
                completion_for(inst, condition, bool(clean[i]))
                if condition == "process"
                else completion_for(inst, condition)
            )
            ...
            if tokenizer.eos_token_id is not None:
                cids += [tokenizer.eos_token_id]
            ids = pids + cids
            if len(ids) > max_length:
                raise ValueError(f"{len(ids)} tokens > --max-length={max_length}")
...
class Collator:
    ...
    def __call__(self, rows):
        ...
        for i, (x, y) in enumerate(rows):
            k = len(x)
            ids[i, :k] = torch.tensor(x)
            labels[i, :k] = torch.tensor(y)
            mask[i, :k] = 1
...
def build_model(args, tok, device):
    model = AutoModelForCausalLM.from_pretrained(
        args.model, dtype=choose_dtype(device), low_cpu_mem_usage=True
    )
...
def parse_generation(text, condition):
    ...
    if condition in {"outcome", "answer_first"}:
        m = ANSWER_RE.search(line.split(":", 1)[1])
        return None, m.group(1) if m else None
    trace, ans = line.rsplit(":", 1)
    m = ANSWER_RE.search(ans)
...
def parse_trace_positionally(trace, depth):
    ...
    for t, token in enumerate(trace.split()[:depth]):
        ...
        if m:
            gates[t], states[t], steps[t] = m.group(1), m.group(2), token
    return gates, states, steps
...
def evaluate_free(model, tok, instances, condition, args, device):
    ...
    for eid, (inst, text) in enumerate(zip(instances, texts)):
        pred_trace, pred_ans = parse_generation(text, condition)
        aok = pred_ans == inst.gold
        ans_ok += int(aok)
        ...
        if condition == "process":
            ...
            gold = [split_step(s) for s in gold_steps]
            pg, ps, pst = parse_trace_positionally(pred_trace, depth)
            et = pred_trace == inst.correct_trace
            es = all(ps[t] == gold[t][1] for t in range(depth))
            exact_trace += int(et)
            exact_states += int(es)
            for t in range(depth):
                gate_ok += int(pg[t] == gold[t][0])
                state_ok += int(ps[t] == gold[t][1])
                step_ok += int(pst[t] == gold_steps[t])
                transitions += 1
    ...
    if condition != "process":
        return {
            ...
            "exact_trace_accuracy": None,
            "exact_state_rollout_accuracy": None,
            "free_step_accuracy": None,
            "free_state_accuracy": None,
            "free_gate_accuracy": None,
            "per_example": per_example,
            "audit": audit,
        }
    return {
        "answer_accuracy": ans_ok / n,
        "exact_trace_accuracy": exact_trace / n,
        "exact_state_rollout_accuracy": exact_states / n,
        "free_step_accuracy": step_ok / transitions,
        "free_state_accuracy": state_ok / transitions,
        "free_gate_accuracy": gate_ok / transitions,
        "per_example": per_example,
        "audit": audit,
    }
...
def local_prefix(inst, t):
    ...
    steps = trace_steps(inst.correct_trace)
    gate, gold_state = split_step(steps[t])
    prefix = inst.prompt + " "
    if t:
        prefix += " ".join(steps[:t]) + " "
    prefix += gate + ">"
...
def encode_local(tok, prefix, candidate):
    ...
    if full != pids + cids:
        raise RuntimeError(
            "Tokenizer boundary changed at local state. Prefix ends at '>' but "
            "encode(prefix+state) != encode(prefix)+encode(state)."
        )
    return pids, cids
    ...
    def __init__(self, instances, tok):
        ...
        for eid, inst in enumerate(instances):
            for t in range(len(trace_steps(inst.correct_trace))):
                prefix, gold, gate = local_prefix(inst, t)
...
class LocalCollator:
    ...
    def __call__(self, rows):
        ...
        ids = torch.full((n, w), self.pad, dtype=torch.long)
        attn = torch.zeros((n, w), dtype=torch.long)
        cmask = torch.zeros((n, w), dtype=torch.bool)
        qids, states = [], []
        for i, r in enumerate(rows):
            k = len(r["ids"])
            ids[i, :k] = torch.tensor(r["ids"])
            attn[i, :k] = 1
            cmask[i, r["start"] : k] = True
...
def score_local_states(model, tok, instances, args, device):
    ...
    scores = {q["query_id"]: {} for q in ds.queries}
    model.eval()
    for b in tqdm(loader, desc="16-way local margins", leave=False):
        ...
        for i, lp in enumerate(seq_lp):
            scores[b["qids"][i]][b["states"][i]] = float(lp)
    rows = []
    for q in ds.queries:
        s = scores[q["query_id"]]
        gold = q["gold_state"]
        if set(s) != set(ALL_STATES):
            raise AssertionError("Missing one or more 16-state candidates")
        best_state = max(s, key=s.get)
        wrong_state = max((x for x in ALL_STATES if x != gold), key=s.get)
        margin = s[gold] - s[wrong_state]
        vals = np.array([s[x] for x in ALL_STATES], dtype=np.float64)
        ...
        rows.append(
            {
                ...
                "best_wrong_state": wrong_state,
                "gold_logprob": s[gold],
                "best_wrong_logprob": s[wrong_state],
                "margin": margin,
                "gold_prob16": float(probs[ALL_STATES.index(gold)]),
                "local_correct": int(margin > 0.0),
...
def aggregate_local(rows, free_per_example, depth):
    groups = {}
    for r in rows:
        groups.setdefault(r["example_id"], []).append(r)
    exrows = []
    for eid in sorted(groups):
        rr = sorted(groups[eid], key=lambda x: x["step"])
        if len(rr) != depth:
            raise AssertionError(f"example {eid}: {len(rr)} rows != depth {depth}")
        exrows.append(
            {
                "example_id": eid,
                "all_local_correct": int(all(r["local_correct"] for r in rr)),
                "min_margin": min(r["margin"] for r in rr),
                "mean_margin": float(np.mean([r["margin"] for r in rr])),
    ...
    return {
        "local_state_accuracy": local_acc,
        "all_local_correct_rate": all_rate,
        "iid_local_prediction": iid_prediction,
        "local_tie_rate": tie_rate,
        "mean_product_gold_prob16": product_prob,
        "min_margin_exact_corr": corr,
        "local_vs_free_state_agreement": agreement,
        "example_rows": exrows,
    }
...
def label_for(condition, rho):
    return condition if condition != "process" else f"rho_{rho:.2f}"


def save_adapter(model, tok, root, condition, rho, seed):
    path = root / label_for(condition, rho) / f"seed_{seed}"
...
def run_one(task, tok, ds, val, condition, rho, seed, args, device):
    ...
    print("=" * 96)
    set_seed(seed)
    model = build_model(args, tok, device)
    trainable, total, frac = trainable_summary(model)
    ...
    label = label_for(condition, rho)
    gen_dir = args.run_dir / "generations"
    gen_dir.mkdir(parents=True, exist_ok=True)
    (gen_dir / f"{label}_seed_{seed}.json").write_text(json.dumps(free["audit"], indent=2))

    local = {
        "local_state_accuracy": None,
        "all_local_correct_rate": None,
        "iid_local_prediction": None,
        "local_tie_rate": None,
        "mean_product_gold_prob16": None,
        "min_margin_exact_corr": None,
        "local_vs_free_state_agreement": None,
    }
    local_n = ""
    if condition == "process":
        local_n = min(args.local_eval_size, len(val))
        local_val = val[:local_n]
    ...
    if args.save_adapters:
        adapter_path = str(save_adapter(model, tok, args.run_dir / "adapters", condition, rho, seed))
    row = {
        "model": args.model,
        "task": task.name,
        "condition": condition if condition != "process" else f"rho={rho:.2f}",
        "rho": "" if rho is None else rho,
        "realized_rho": "" if ds.realized_rho is None else ds.realized_rho,
        "seed": seed,
        "train_size": len(ds),
        "val_size": len(val),
        "local_eval_size": local_n,
        "steps": args.steps,
        "batch_size": args.batch_size,
    ...
    if condition == "process":
        print(
            f"free exact trace={100*free['exact_trace_accuracy']:.2f}% | exact states={100*free['exact_state_rollout_accuracy']:.2f}% | "
            f"free state={100*free['free_state_accuracy']:.2f}% | free gate={100*free['free_gate_accuracy']:.2f}%"
        )
        print(
            f"16-way local state={100*local['local_state_accuracy']:.2f}% | all-local-correct={100*local['all_local_correct_rate']:.2f}% | "
...
def summarize(path):
    ...
    cols = [
        ...
        "exact_state_rollout_accuracy",
        "free_state_accuracy",
        "local_state_accuracy",
        "all_local_correct_rate",
        "iid_local_prediction",
        "min_margin_exact_corr",
    ]
    print(df[cols].to_string(index=False))
    p = df[df["rho"].notna()].copy()
    if len(p) > 1:
        pred = p["all_local_correct_rate"].astype(float).to_numpy()
        obs = p["exact_state_rollout_accuracy"].astype(float).to_numpy()
...
def parse_args(argv=None):
    ...
    p = argparse.ArgumentParser(
        parents=[pre],
        prog="python -m src lora",
        description=__doc__,
        formatter_class=argparse.ArgumentDefaultsHelpFormatter,
    )
    p.add_argument("--model", default=cfg.get("model", "HuggingFaceTB/SmolLM2-135M"))
    ...
    p.add_argument(
        "--assignment-seed", "--ratio-seed", dest="assignment_seed", type=int,
        default=int(cfg.get("assignment_seed", cfg.get("ratio_seed", 777))),
    )
    p.add_argument("--batch-seed", type=int, default=int(cfg.get("batch_seed", 12345)))
    p.add_argument("--steps", type=int, default=int(cfg.get("steps", 800)))
    p.add_argument("--batch-size", type=int, default=int(cfg.get("batch_size", 16)))
    p.add_argument("--eval-batch-size", type=int, default=int(cfg.get("eval_batch_size", 32)))
    p.add_argument("--local-batch-size", type=int, default=int(cfg.get("local_batch_size", 128)))
    ...
    p.add_argument("--lora-rank", type=int, default=int(cfg.get("lora_rank", 8)))
    p.add_argument("--full-finetune", action="store_true", default=bool(cfg.get("full_finetune", False)))
    p.add_argument("--max-length", type=int, default=int(cfg.get("max_length", 384)))
    p.add_argument("--process-max-new-tokens", type=int, default=int(cfg.get("process_max_new_tokens", 128)))
    p.add_argument("--outcome-max-new-tokens", type=int, default=int(cfg.get("outcome_max_new_tokens", 20)))
    p.add_argument("--audit-examples", type=int, default=int(cfg.get("audit_examples", 8)))
    p.add_argument("--save-adapters", action=argparse.BooleanOptionalAction, default=bool(cfg.get("save_adapters", True)))
    p.add_argument("--output", type=Path, default=Path(cfg.get("output", "results/paper/smollm/smollm_trace.csv")))
    p.add_argument("--run-dir", type=Path, default=Path(cfg.get("run_dir", "results/paper/smollm/artifacts")))
...
def run(args):
    if any(r < 0 or r > 1 for r in args.rhos):
        raise SystemExit("every rho must be in [0,1]")
    if args.local_eval_size < 1:
        raise SystemExit("--local-eval-size must be positive")
    if args.task not in TASKS:
        raise KeyError(f"Unknown Trace task: {args.task}")
    if not args.task.startswith("boolean_circuit_"):
        raise ValueError("16-way local evaluator is for boolean_circuit_* tasks")
    ...
    for condition, rho in conditions:
        ...
        for seed in args.seeds:
            run_one(task, tok, ds, val, condition, rho, seed, args, device)
        del ds
        gc.collect()
